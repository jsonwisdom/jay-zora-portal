...
for (const name of workflowFiles) {
  ...
  if (pushesCanonical) {
    fail("AUTOMATION_PUSHES_CANONICAL", `${name} contains an automated git push to ${DEFAULT_BRANCH}. Canonical Git promotion must go through an explicit branch/PR decision.`);
  }
}
...
if (!fs.existsSync(exportWorkflow)) {
  ...
  if (/(^|\n)\s*pull_request\s*:/m.test(text)) {
    fail("EXPORT_LISTENS_TO_PR", "Inventory export must not trigger on pull_request; PR validation is a separate read-only lane.");
  }
  if (!/contents\s*:\s*read/m.test(text)) {
    fail("EXPORT_NOT_READ_ONLY", "Inventory export must declare contents: read.");
  }
  if (/contents\s*:\s*write/m.test(text)) {
    fail("EXPORT_WRITE_SCOPE", "Inventory export must not have contents: write.");
  }
  if (/git\s+push/m.test(text)) {
    fail("EXPORT_PUSH_PRESENT", "Inventory export must be artifact-only and must not push Git refs.");
  }
  if (!/actions\/upload-artifact@/m.test(text)) {
    fail("EXPORT_ARTIFACT_MISSING", "Artifact-only export must publish its CSV/manifest as an Actions artifact.");
  }
  observe("EXPORT_ARTIFACT_ONLY", "Inventory export is read-only and publishes generated bytes as an Actions artifact.");
}
...
if (!fs.existsSync(prWorkflow)) {
  ...
  if (!/merge-base\s+--is-ancestor\s+origin\/live-zora-ingestion\s+HEAD/.test(text)) {
    fail("PR_VALIDATOR_NO_BASE_ANCESTRY_GATE", "PR validator must require the canonical tip to be an ancestor of the PR head.");
  }
  observe("PR_VALIDATOR_EXACT_HEAD", "Read-only PR validator is bound to the exact pull-request head SHA and a synchronized canonical base.");
}
